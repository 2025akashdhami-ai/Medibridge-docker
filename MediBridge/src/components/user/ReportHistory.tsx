...
export default function ReportHistory() {
  ...
  const downloadReport = (report: Report) => {
    ...
    doc.setFontSize(10);
    doc.text(`Generated: ${new Date(report.createdAt).toLocaleDateString()}`, 20, 30);
    
    doc.setFontSize(14);
    doc.text('Symptoms Described:', 20, 45);
    doc.setFontSize(10);
    const splitSymptoms = doc.splitTextToSize(report.symptoms_text, 170);
    doc.text(splitSymptoms, 20, 55);
    
    doc.setFontSize(14);
    doc.text('Preliminary Diagnosis:', 20, 80);
    doc.setFontSize(12);
    doc.text(report.predicted_disease, 20, 90);
    doc.setFontSize(10);
    doc.text(`Confidence: ${report.confidence}%`, 20, 97);
    ...
    doc.save(`symptom-report-${report._id}.pdf`);
  };
